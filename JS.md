...
- `{ }`: is mainly used for `for code block`
- `( )`: is mainly used for `for functions and pass paramters`
- `[ ]`: is mainly used for `for array`
...
Anything in javascript between `" "` or `' '` or back ticks `string` is called a string
...
array.lenght; // returns number of elements in the array
console.log(array.lenght); // prints number of elements in the array
//
```
...
2. `.fliter`: returns a filtered copy of the array that is based on a condition that you return from the function you pass it.
...
4. `.includes`: looks exactly like `.filter`, but it only returns the first element that matches the condition
...
array.includes((e) => e === 20); // returns 20
array.find((e) => e === 20); // returns true
array.reduce((prev, current) => prev + current); // returns the sum of the array (210)
```

# Objects

The structure of object is a pair of `animel : " cat"`
